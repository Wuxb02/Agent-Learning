...
from AgentScope_practice.tools.Prompt import get_prompt
...
from AgentScope_practice.agents.game_roles import GameRoles
from AgentScope_practice.agents.structured_output_cn import (
    DiscussionModelCN,
    get_vote_model_cn,
    WitchActionModelCN,
    get_seer_model_cn,
...
from AgentScope_practice.utils.Tool import (
    check_winning_cn,
    majority_vote_cn,
    get_chinese_name,
    format_player_list,
...
class ThreeKingdomsWerewolfGame:
    ...
    async def create_player(self, role: str, character: str) -> ReActAgent:
        ...
        agent = ReActAgent(
            name=name,
            sys_prompt=get_prompt("agentscope_role.j2", {
                "character": character,
                "role": role
            }),
            ...
            for _ in range(MAX_DISCUSSION_ROUND):
                for wolf in self.werewolves:
                    ...
                    if msg and msg.metadata:
                        print(f"[{wolf.name}] {msg.metadata.get('key_evidence', '')}")
...
async def main():
    ...
    logger.remove()  # 移除所有 loguru handler，彻底屏蔽 AgentScope 的打印
